...
idx = 6051; % dataset -> RSS values. This is the index of the optimal parameters from hyperparameters tuning 
...
Percent_time_conect_LiFi_RL_HO_matrix = zeros(length(speeds_vector),length(lamda_vector));
...
Percent_time_blockage_RL_HO_matrix = zeros(length(speeds_vector),length(lamda_vector));
...
Blockage_rate_RL_HO_matrix = zeros(length(speeds_vector),length(lamda_vector));
...
Throughput_matrix_RL_HO = zeros(length(speeds_vector),length(lamda_vector));
Avr_HHO_rate_matrix_RL_HO = zeros(length(speeds_vector),length(lamda_vector));
Avr_VHO_rate_matrix_RL_HO = zeros(length(speeds_vector),length(lamda_vector));
Percent_time_conect_WiFi_RL_HO_matrix = zeros(length(speeds_vector),length(lamda_vector));
Avr_N_block_RL_HO_matrix = zeros(length(speeds_vector),length(lamda_vector));
...
Avr_Outage_prob_RL_HO_matrix = zeros(length(speeds_vector),length(lamda_vector));
...
%         q_inital_value = 0.5;
        ...
        for ite = 1 : N_ite
            ...
            best_AP_RL_HO = 1; % Indicates the best target AP that it is being tracked. If it stays the best for 3 t_TTT, will become Host_AP
            Host_AP_RL_HO = 1; % Initially, the main user is connected to LiFi AP1
            prev_Host_AP_RL_HO = 1;
            flag_t_TTT_RL_HO = 0; % Flag to indicate if t_TTT was already triggered
            delta_to_trigger_t_TTT_RL_HO = 1; % at least 1 dB higher 
            t_TTT_RL_HO = 0; % counter time to trigger HO algorithm
            N_VHO_RL_HO = 0;
            N_HHO_RL_HO = 0;
            N_block_RL_HO = 0;
            N_blockages_RL_HO = 0;
            Blockage_vector_RL_HO = []; % stores the blockages per iteration. 0 -> blockage, 1 -> no blockage
            t_TTT_vector_RL_HO = []; % stores the value of t_TTT counter for each iteration
            HO_algorithm_triggers_RL_HO = []; % for each iteration, it stores 1 -> HO algorithm was triggered or 0 -> HO algorithm wasn't triggered
            best_AP_vector_RL_HO = []; % stores the best AP
            Host_AP_vector_RL_HO = [1];
            SNR_Host_AP_vector_RL_HO = [];
            SINR_Host_AP_vector_RL_HO = [];
            LiFi_blockages_RL_HO = [];
%             user_traj_predict_vector_RL_HO = [];
%             HO_failure_vector_RL_HO = [];
            ...
            for k = 1 : N_r
                ...
                pos_ue_k = pos_ue_trajectory(k, :); % get the waypoint. Note this is the next point. The current one is denoted k-1. For first excursion k-1 is the (0,0)
                ...
                while t_move <= D_k/speed
                    ...
                    if SINR_all_APs_dB(1,current_best_AP_SINR_indx) >= SINR_all_APs_dB(1,Host_AP_STD_LTE) + delta_to_trigger_t_TTT % at least 1 dB of difference
                       ...
                       if (Host_AP_STD_LTE ~= 17) && (SINR_all_APs_dB(1,Host_AP_STD_LTE) == -30) % blockage
                            ...
                            if t_TTT_STD_LTE == 2 % The AP is the best for 260ms (TTT)
                               prev_Host_AP_STD_LTE = Host_AP_STD_LTE;
                    ...
                    if SINR_all_APs_dB(1,current_best_AP_SINR_indx) >= SINR_all_APs_dB(1,Host_AP_Smart_HO) + delta_to_trigger_t_TTT % at least 1 dB of difference
                        if (Host_AP_Smart_HO ~= 17) && (SINR_all_APs_dB(1,Host_AP_Smart_HO) == -30) % blockage
                            ...
                            if t_TTT_Smart_HO == 2 % The AP is the best for 260ms (TTT)
...
%                                    prev_Host_AP_Smart_HO = Host_AP_Smart_HO;
                    ...
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
                    %%% Proposed Algorithm (RL-HO)
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    
                    % Counting blockages
                    if (Host_AP_RL_HO ~= 17) && (SINR_all_APs_dB(1,Host_AP_RL_HO) == -30) % blockage   
                        Blockage_vector_RL_HO = [Blockage_vector_RL_HO 0];
                    else
                        Blockage_vector_RL_HO = [Blockage_vector_RL_HO 1];
                    end
                    
                    %%% Cheking if it is necessary to make a HO decision
                    
                    if SINR_all_APs_dB(1,current_best_AP_SINR_indx) >= SINR_all_APs_dB(1,Host_AP_RL_HO) + delta_to_trigger_t_TTT_RL_HO % at least 1 dB of difference
                        
                        if not(Host_AP_RL_HO==17) && (SINR_all_APs_dB(1,Host_AP_RL_HO) == -30) 
                            ...
                            if Host_AP_RL_HO==17
                                ...
                                % Check HO failure
                                SINR_current_host_AP = SINR_all_APs_dB(1,Host_AP_RL_HO);
                                if SINR_current_host_AP > SINR_HO_failure_RL_HO
                                    HO_failure_vector_RL_HO = [HO_failure_vector_RL_HO; 1]; 
                                    HO_failure = 1; % no failure
                                else
                                    HO_failure_vector_RL_HO = [HO_failure_vector_RL_HO; 0];
                                    HO_failure = 0;
                                end
                                
                                % User satisfaction degree in terms of data rate
                                if Host_AP_RL_HO == 17 % WiFi link
                                    new_datarate = B_WiFi * log2(1 + sinr_wifi_matrix(1,1));
                                    new_datarate_Mbps = new_datarate/1000000;
                                else % LiFi link
                                    new_datarate = B_LiFi * log2(1 + (exp(1)/2*pi)* sinr_lifi_matrix_total(1,Host_AP_RL_HO));
                                    new_datarate_Mbps = new_datarate/1000000;
                                end 
%                                
                                ...
                                % The HO Cost depends on the consequnces of the previous action in previous state
                                
                                if prev_action == 3 % Do Nothing
                                    HO_cost = HO_Cost_No_HO; % Do Nothing
                                else
                                    if prev_Host_AP_RL_HO ~= 17 && Host_AP_RL_HO == 17 % LiFi to WiFi
                                       HO_cost = HO_Cost_VHO;
                                    elseif prev_Host_AP_RL_HO == 17 && Host_AP_RL_HO ~= 17 % WiFi to LiFi
                                       HO_cost = HO_Cost_VHO;
                                    elseif prev_Host_AP_RL_HO ~= 17 && Host_AP_RL_HO ~= 17 % LiFi to LiFi
                                       HO_cost = HO_Cost_HHO;
                                    elseif prev_Host_AP_RL_HO == 17 && Host_AP_RL_HO == 17 % WiFi to WiFi
                                       HO_cost = HO_Cost_HHO;
                                    end 
                                end
                                ...
                                if SINR_all_APs_dB(1,Host_AP_RL_HO) > SINR_user_requirement 
                                ...
                                input_ML = [Host_AP_RL_HO SNR_all_APs_dB(1,1:16) delta_SNR];

                                % Call ML algorithm to make prediction
                                user_traj_predict = xgboost_test(input_ML,best_model,0);
                                user_traj_predict_vector_RL_HO = [user_traj_predict_vector_RL_HO user_traj_predict];
                                
                                switch user_traj_predict
                                    case 0 % walking to the wall
                                        if user_satisfaction == 1 && Host_AP_RL_HO ~= 17 
                                           current_state = 1;
                                           disp('LiFi, walking to wall, user satisfied')
                                        elseif user_satisfaction == 0 && Host_AP_RL_HO ~= 17 
                                           current_state = 4; 
                                           disp('LiFi, walking to wall, user not satisfied')
                                        elseif user_satisfaction == 1 && Host_AP_RL_HO == 17 
                                           current_state = 7;
                                ...
                                % Make a HO decision
                                Host_AP_RL_HO = 17;    % Switch to WiFi
                                prev_Host_AP_RL_HO = Host_AP_RL_HO;
                                prev_state = current_state;
                                prev_action = 2;
                                prev_datarate = new_datarate;                              
                                actions_vector_RL_HO = [actions_vector_RL_HO 2];
                                N_VHO_RL_HO = N_VHO_RL_HO + 1;
                                t_TTT_RL_HO = 0;
                                flag_t_TTT_RL_HO = 0;
                                HO_algorithm_triggers_RL_HO = [HO_algorithm_triggers_RL_HO; 1]; % list of HOs for each iteration 1-> VHO
                                N_block_RL_HO = N_block_RL_HO + 1;
                            end   

                        else   % No blockage
                            % Mechanism to trigger the t_TTT counter (as in LTE)
                            if flag_t_TTT_RL_HO == 1 % t_TTT was already triggered
                                if best_AP_RL_HO == current_best_AP_SINR_indx
                                    t_TTT_RL_HO = t_TTT_RL_HO + 1;
                                else
                                    t_TTT_RL_HO = 0;
                                    flag_t_TTT_RL_HO = 0;
                                end
                                ...
                                flag_t_TTT_RL_HO = 1;
                                t_TTT_RL_HO = 1;
                                best_AP_RL_HO = current_best_AP_SINR_indx;
                                SNR_all_APs_dB_when_t_TTT_triggered = SNR_all_APs_dB;
                            end
                            t_TTT_vector_RL_HO = [t_TTT_vector_RL_HO; t_TTT_RL_HO];
                            
                            if t_TTT_RL_HO == Threshold_t_TTT_RL_HO
                                t_TTT_RL_HO = 0;
                                flag_t_TTT_RL_HO = 0;
                                ...
                                % The HO Cost depends on the consequnces of previous action in previous state
...
%                                 [val, action] = max(q_table(current_state,:));% greedy policy
                                ...
                                elseif prev_Host_AP_RL_HO == 17 && Host_AP_RL_HO ~= 17 % WiFi to LiFi
                                   ...
                                   HO_algorithm_triggers_RL_HO = [HO_algorithm_triggers_RL_HO; 1]; % list of HOs for each iteration 1-> VHO
                                elseif prev_Host_AP_RL_HO ~= 17 && Host_AP_RL_HO ~= 17 % LiFi to LiFi
                                   N_HHO_RL_HO = N_HHO_RL_HO + 1;
                                   HO_algorithm_triggers_RL_HO = [HO_algorithm_triggers_RL_HO; 2]; % list of HOs for each iteration 2-> HHO
                                elseif prev_Host_AP_RL_HO == 17 && Host_AP_RL_HO == 17 % WiFi to WiFi
                                   HO_algorithm_triggers_RL_HO = [HO_algorithm_triggers_RL_HO; 0]; % list of HOs for each iteration 0-> No HO                           
                                end     
                                ...
                                HO_algorithm_triggers_RL_HO = [HO_algorithm_triggers_RL_HO; 0];
                                actions_vector_RL_HO = [actions_vector_RL_HO 3];
                            end
                        end
                    else
                        t_TTT_vector_RL_HO = [t_TTT_vector_RL_HO; 0];
                        HO_algorithm_triggers_RL_HO = [HO_algorithm_triggers_RL_HO; 0];
                        flag_t_TTT_RL_HO = 0;
                        t_TTT_RL_HO = 0;
                        actions_vector_RL_HO = [actions_vector_RL_HO 3];
                    ...
                    Host_AP_vector_RL_HO = [Host_AP_vector_RL_HO Host_AP_RL_HO];
                    SINR_Host_AP_vector_RL_HO = [SINR_Host_AP_vector_RL_HO SINR_all_APs_dB(1,Host_AP_RL_HO)];                                      
                    N_HO_failures_RL_HO = length(HO_failure_vector_RL_HO) - sum(HO_failure_vector_RL_HO);
            ...
            %Proposed RL-HO scheme
            Thr_vector_Mbps_RL_HO = zeros(1,n-1);

            for k_thr = 1: n-1
                if Host_AP_vector_RL_HO(1,k_thr) == 17  % WiFi link
                    new_datarate = B_WiFi * log2(1 + sinr_wifi_matrix_watt_ite(k_thr,1)*Blockage_vector_RL_HO(1,k_thr));
                    new_datarate_Mbps = new_datarate/1000000;
                    Thr_vector_Mbps_RL_HO(1,k_thr) = new_datarate_Mbps;

                else  % LiFi link
                    new_datarate = B_LiFi * log2(1 + (exp(1)/2*pi)* sinr_lifi_matrix_watt_ite_total_v1(k_thr,Host_AP_vector_RL_HO(1,k_thr))*Blockage_vector_RL_HO(1,k_thr));
                    new_datarate_Mbps = new_datarate/1000000;
                    Thr_vector_Mbps_RL_HO(1,k_thr) = new_datarate_Mbps;
                end 
            ...
            Avr_thr_RL_HO = sum(Thr_vector_Mbps_RL_HO)/length(Thr_vector_Mbps_RL_HO);


            for kk_thr = 1: n-1
                if kk_thr + 3 < n-1
                    if HO_algorithm_triggers_RL_HO(kk_thr,1) == 1
                        Thr_vector_Mbps_RL_HO(1,kk_thr:kk_thr+3) = 0;
                    elseif HO_algorithm_triggers_RL_HO(kk_thr,1) == 2
                        Thr_vector_Mbps_RL_HO(1,kk_thr:kk_thr+1) = 0;
                    end   
                end
            end

            Avr_thr_RL_HO_with_Overhead = sum(Thr_vector_Mbps_RL_HO)/length(Thr_vector_Mbps_RL_HO);
            ...
            Index_Blockage_vector_RL_HO = find(Blockage_vector_RL_HO==0);
            Host_AP_vector_RL_HO_aux = Host_AP_vector_RL_HO;
            Host_AP_vector_RL_HO_aux(1,Index_Blockage_vector_RL_HO) = 0;
          
  
            SINR_Host_AP_vector_RL_HO_aux = SINR_Host_AP_vector_RL_HO;
            SINR_Host_AP_vector_RL_HO_aux(1,Index_Blockage_vector_RL_HO) = -30;
            
            N_Conect_WiFi_RL_HO = length(find(Host_AP_vector_RL_HO_aux==17)); % finds how many times it was connected to WiFi
            Percent_time_conect_WiFi_RL_HO = N_Conect_WiFi_RL_HO/(n-1);
            Percent_time_blockage_RL_HO = length(Index_Blockage_vector_RL_HO )/(n-1);
            Percent_time_conect_LiFi_RL_HO = 1 - Percent_time_conect_WiFi_RL_HO - Percent_time_blockage_RL_HO;
            ...
            Outage_prob_RL_HO = length(find(SINR_Host_AP_vector_RL_HO < Threshold_coverage))/(n-1);
            ...
            N_HHO_RL_HO_per_ite(1,ite) = N_HHO_RL_HO;
            N_VHO_RL_HO_per_ite(1,ite) = N_VHO_RL_HO; 
            Thr_RL_HO_per_ite(1,ite) = Avr_thr_RL_HO_with_Overhead;
            ...
            Percent_time_conect_WiFi_RL_HO_ite(1,ite) = Percent_time_conect_WiFi_RL_HO;
            ...
            Percent_time_conect_LiFi_RL_HO_ite(1,ite) = Percent_time_conect_LiFi_RL_HO;
            ...
            Percent_time_blockage_RL_HO_ite(1,ite) = Percent_time_blockage_RL_HO;
            Outage_prob_STD_LTE_per_ite(1,ite) = Outage_prob_STD_LTE;
            Outage_prob_Smart_HO_per_ite(1,ite) = Outage_prob_Smart_HO;
            Outage_prob_RL_HO_per_ite(1,ite) = Outage_prob_RL_HO;
            ...
            N_block_RL_HO_per_ite(1,ite) = N_block_RL_HO;
        ...
        Avr_N_HHO_RL_HO = sum(N_HHO_RL_HO_per_ite)/length(N_HHO_RL_HO_per_ite);
        Avr_N_VHO_RL_HO = sum(N_VHO_RL_HO_per_ite)/length(N_VHO_RL_HO_per_ite);

        Avr_HHO_rate_vector_RL_HO = Avr_N_HHO_RL_HO/(Avr_N_steps*T_c_theta);
        Avr_VHO_rate_vector_RL_HO = Avr_N_VHO_RL_HO/(Avr_N_steps*T_c_theta);

        Avr_Thr_RL_HO = sum(Thr_RL_HO_per_ite)/length(Thr_RL_HO_per_ite);
        Avr_Percent_time_conect_WiFi_RL_HO = sum(Percent_time_conect_WiFi_RL_HO_ite)/length(Percent_time_conect_WiFi_RL_HO_ite);
        Avr_Percent_time_conect_LiFi_RL_HO = sum(Percent_time_conect_LiFi_RL_HO_ite)/length(Percent_time_conect_LiFi_RL_HO_ite);
        Avr_Percent_time_blockage_RL_HO = sum(Percent_time_blockage_RL_HO_ite)/length(Percent_time_blockage_RL_HO_ite);
        ...
        Throughput_matrix_RL_HO(spd,lmd) = Avr_Thr_RL_HO;
        Avr_HHO_rate_matrix_RL_HO(spd,lmd) = Avr_HHO_rate_vector_RL_HO;
        Avr_VHO_rate_matrix_RL_HO(spd,lmd) = Avr_VHO_rate_vector_RL_HO;
        ...
        Avr_Outage_prob_RL_HO = sum(Outage_prob_RL_HO_per_ite)/length(Outage_prob_RL_HO_per_ite);
        ...
        Avr_Outage_prob_RL_HO_matrix(spd,lmd) = Avr_Outage_prob_RL_HO;
        ...
        Percent_time_conect_WiFi_RL_HO_matrix(spd,lmd) = Avr_Percent_time_conect_WiFi_RL_HO;
        ...
        Percent_time_conect_LiFi_RL_HO_matrix(spd,lmd) = Avr_Percent_time_conect_LiFi_RL_HO;
        ...
        Percent_time_blockage_RL_HO_matrix(spd,lmd) = Avr_Percent_time_blockage_RL_HO;
        ...
        Avr_N_block_RL_HO = sum(N_block_RL_HO_per_ite)/length(N_block_RL_HO_per_ite);
    
        Avr_N_block_STD_LTE_matrix(spd,lmd) = Avr_N_block_STD_LTE;
        Avr_N_block_Smart_HO_matrix(spd,lmd) = Avr_N_block_Smart_HO;
        Avr_N_block_RL_HO_matrix(spd,lmd) = Avr_N_block_RL_HO;
...
Thr_matrix = [Throughput_matrix_STD_LTE';Throughput_matrix_Smart_HO';Throughput_matrix_RL_HO'];
user_speed = speeds_vector;
figure;
...
HHO_rate_matrix = vertcat(Avr_HHO_rate_matrix_STD_LTE(3,:),Avr_HHO_rate_matrix_Smart_HO(3,:),Avr_HHO_rate_matrix_RL_HO(3,:));
VHO_rate_matrix = vertcat(Avr_VHO_rate_matrix_STD_LTE(3,:),Avr_VHO_rate_matrix_Smart_HO(3,:),Avr_VHO_rate_matrix_RL_HO(3,:));
